...
x_columns = all_columns_for_xgb[:-1]

combined_df = combined_df[all_columns_for_xgb]
...
print("o")
print(np.median(agg_data['Temp']))

#high - female
#low - male
agg_data_high = agg_data[agg_data['Gender'] == 0]
agg_data_low= agg_data[agg_data['Gender'] == 1]


agg_data_high.reset_index(inplace=True)
agg_data_low.reset_index(inplace=True)
print("len of high temp:" , len(agg_data_high))
print("len of low temp:" , len(agg_data_low))
...
print("number of predicted sick- f temp: ", len([x for x in y_pred if x ==1]))
f1 = f1_score(y_test_high, y_pred)
...
y_pred = loaded_model.predict(dtest)
y_pred = [int(round(x)) for x in y_pred]
print("number of predicted sick- m temp: ", len([x for x in y_pred if x ==1]))
f1 = f1_score(y_test_low, y_pred)
print("F1-score: {:.2f}".format(f1))